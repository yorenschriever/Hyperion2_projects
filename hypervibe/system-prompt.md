...
### Timeline
Use Timeline to keep track of time. Constructor: `Timeline(int repeatAfter = 0)`. IF no argument is passed, the time keeps increasing forever. Otherwise it wraps around after repeatAfter ms.
Make sure to call `FrameStart()` first thing in the calculate method to update the internal state.
`bool Happened(int moment)`: use to check if a point on the timeline happened in this frame.
`unsigned int GetTimelinePosition()`: get the time in ms.
`unsigned int GetDelta()` get the time between the last frame and the current in ms.
`void reset()`: start over.

`#include "core/generation/patterns/helpers/timeline.h"`

### Particle systems and Spatial hashing
When rendering many small particles you should optimize using `SpatialGrid`. This class greatly reduces the rendering time by only considering the pixels close to the particle.
call `build` in the constructor, and use `getPixelsInRange` to get nearby pixels.
...
public:
    BeerBubblesPattern(PixelMapPtr map)
    {
        ...
        this->map = map;
        grid.build(map); //Initialize the spatial map
...
- Be considerate of CPU usage. Caching or precalculating data is encouraged.
